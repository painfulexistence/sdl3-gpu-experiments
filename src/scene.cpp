...
void Scene::UploadNode(const std::shared_ptr<Node>& node, SDL_GPUDevice* device) {
    const auto CreateAndUploadBuffer = [device](const void* data, size_t size,  SDL_GPUBufferUsageFlags usage) -> std::shared_ptr<SDL_GPUBuffer> {
    ...
    const auto CreateAndUploadTexture = [device](const Image& img) -> SDL_GPUTexture* {
        ...
        if (img.component == 1) {
            ...
            SDL_Log("Unknown component count: %d", img.component);
            return nullptr;
        }
        auto texture = SDL_CreateGPUTexture(device, &texCreateInfo);
        SDL_GPUTransferBufferCreateInfo transferInfo = {
            .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
            .size = static_cast<Uint32>(img.pixels.size())
        };
        SDL_GPUTransferBuffer* transferBuffer = SDL_CreateGPUTransferBuffer(device, &transferInfo);

        void* transferData = SDL_MapGPUTransferBuffer(device, transferBuffer, false);
        memcpy(transferData, img.pixels.data(), img.pixels.size());
        SDL_UnmapGPUTransferBuffer(device, transferBuffer);
    ...
    if (node->meshGroup) {
        for (const auto& mesh : node->meshGroup->meshes) {
            ...
            mesh->vbos.push_back(CreateAndUploadBuffer(positions.data(), positions.size() * sizeof(glm::vec3), SDL_GPU_BUFFERUSAGE_VERTEX));
            mesh->vbos.push_back(CreateAndUploadBuffer(uvs.data(), uvs.size() * sizeof(glm::vec2), SDL_GPU_BUFFERUSAGE_VERTEX));
            mesh->vbos.push_back(CreateAndUploadBuffer(normals.data(), normals.size() * sizeof(glm::vec3), SDL_GPU_BUFFERUSAGE_VERTEX));
            if (tangents.size() > 0) {
                mesh->vbos.push_back(CreateAndUploadBuffer(tangents.data(), tangents.size() * sizeof(glm::vec3), SDL_GPU_BUFFERUSAGE_VERTEX));
            }

            const auto& indices = mesh->indices;
            if (indices.size() > 0) {
                mesh->ebo = CreateAndUploadBuffer(indices.data(), indices.size() * sizeof(Uint32), SDL_GPU_BUFFERUSAGE_INDEX);
            }

            // if (mesh->material) {
                if (mesh->material->albedoMap != nullptr) {
                    mesh->material->albedoMap->texture = CreateAndUploadTexture(*mesh->material->albedoMap);
                }
                if (mesh->material->metallicRoughnessMap != nullptr) {
                    mesh->material->metallicRoughnessMap->texture = CreateAndUploadTexture(*mesh->material->metallicRoughnessMap);
                }
                if (mesh->material->normalMap != nullptr) {
                    mesh->material->normalMap->texture = CreateAndUploadTexture(*mesh->material->normalMap);
                }
                if (mesh->material->occlusionMap != nullptr) {
                    mesh->material->occlusionMap->texture = CreateAndUploadTexture(*mesh->material->occlusionMap);
                }
                if (mesh->material->emissiveMap != nullptr) {
                    mesh->material->emissiveMap->texture = CreateAndUploadTexture(*mesh->material->emissiveMap);
                }

                mesh->material->pipelineInfo.target_info = {
                    .color_target_descriptions = (SDL_GPUColorTargetDescription[]){{
                        .format = SDL_GPU_TEXTUREFORMAT_R16G16B16A16_FLOAT,
                    }},
                    .num_color_targets = 1,
                };
                mesh->material->pipelineInfo.vertex_input_state = (SDL_GPUVertexInputState){
                ...
                if (mesh->material->pipelineInfo.primitive_type != SDL_GPU_PRIMITIVETYPE_TRIANGLELIST) {
                    SDL_Log("Primitive type is not triangle list");
                    continue;
                }
                mesh->material->pipelineInfo.multisample_state = {
                    .sample_count = SDL_GPU_SAMPLECOUNT_4
                };

                mesh->material->pipeline = std::shared_ptr<SDL_GPUGraphicsPipeline>(
                    SDL_CreateGPUGraphicsPipeline(device, &mesh->material->pipelineInfo),
    ...
    for (const auto& node : nodes) {
        if (node->meshGroup) {
            for (const auto& mesh : node->meshGroup->meshes) {
                ...
                // }
                if (mesh->material) {
                    if (mesh->material->albedoMap && mesh->material->albedoMap->texture) {
                        SDL_ReleaseGPUTexture(device, mesh->material->albedoMap->texture);
                    }
                    if (mesh->material->metallicRoughnessMap && mesh->material->metallicRoughnessMap->texture) {
                        SDL_ReleaseGPUTexture(device, mesh->material->metallicRoughnessMap->texture);
                    }
                    if (mesh->material->normalMap && mesh->material->normalMap->texture) {
                        SDL_ReleaseGPUTexture(device, mesh->material->normalMap->texture);
                    }
                    if (mesh->material->occlusionMap && mesh->material->occlusionMap->texture) {
                        SDL_ReleaseGPUTexture(device, mesh->material->occlusionMap->texture);
                    }
                    if (mesh->material->emissiveMap && mesh->material->emissiveMap->texture) {
                        SDL_ReleaseGPUTexture(device, mesh->material->emissiveMap->texture);
                    }
                    // if (mesh->material->pipeline) {
                    //     SDL_ReleaseGPUGraphicsPipeline(device, mesh->material->pipeline.get());
                    // }
                }
            }
        }
